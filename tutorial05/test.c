...
static void test_parse_array() {
	...
	for(i = 0; i < 4; ++i) {
		lept_value *pa = lept_get_array_element(&v, i);
    	EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(pa));
		EXPECT_EQ_SIZE_T(i, lept_get_array_size(pa));
		for(j = 0; j < i; ++j){
			lept_value* pb = lept_get_array_element(pa, j);
			EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(pb));
			EXPECT_EQ_DOUBLE((double)j, lept_get_number(pb));
		}	
	}
    lept_free(&v);
}
